...
int compare(const void *a, const void *b)
{
    ...
    return node_b->num - node_a->num;
}

// int check(struct password *pw)
// {
//     FILE *fp;
//     int ans = 0;
//     fp = fopen("b.txt", "r");
...
//         c[strcspn(c, "\n")] = '\0';
//         char *str = c;
//         int password = atoi(str);
//         char *ptr;
//         char *word;

//         ptr = strchr(c, ' ');
//         ptr += 1;
//         word = strdup(strchr(c, ' ') + 1);
//         unsigned long hash = hashcode(word) % (pw->cursize);
//         node *current = pw->hashmap[hash];
//         while (current != NULL)
//         {
//             if (!strcmp(word, current->word) && password == current->num)
//             {
//                 ans += 1;
//             }
...
{
    unsigned long hash = hashcode(str) % (pw->cursize);
