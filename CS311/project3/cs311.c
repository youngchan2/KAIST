...
int main(int argc, char *argv[]) {
  ...
  /* Error Checking */
  if (argc < 2) {
    printf("Usage: %s [-bpt] [-nof] [-m addr1:addr2] [-d] [-p] [-n num_instr] "
           "inputBinary\n",
           argv[0]);
    exit(1);
  }
  ...
  while (count != argc - 1) {
    if (strcmp(argv[count], "-m") == 0) {
      ...
      pipe_dump_set = 1;
    else if (strcmp(argv[count], "-bpt") == 0)
      // first predicdtion to 't'aken
      BR_BIT = TRUE;
    else if (strcmp(argv[count], "-nof") == 0)
      FORWARDING_BIT = FALSE;
    else {
      printf(
          "Usage: %s [-m addr1:addr2] [-d] [-p] [-n num_instr] inputBinary\n",
          argv[0]);
      exit(1);
    }
    count++;
